...
// colors
const COLORS_TO_PICK = ['#00AB55', '#000000', '#FFFFFF', '#FFC0CB', '#FF4842', '#1890FF', '#94D82D', '#FFC107'];
        ...
        <>
            ...
            <DrawerContentCard title='Gender'>
                <FormGroup>
                    {['Men', 'Women', 'Kids'].map((el, idx) => (
                        <FormControlLabel key={el + idx} control={<Checkbox />} label={el} />
                    ))}
                </FormGroup>
            </DrawerContentCard>
            ...
            <DrawerContentCard title='Category'>
                <FormControl component='fieldset'>
                    <RadioGroup aria-label='category' defaultValue='none' name='category-name'>
                        {['All', 'Shoes', 'Apparel', 'Accessories'].map((el, idx) => (
                            <FormControlLabel key={el + idx} value={el.toLowerCase()} control={<Radio />} label={el} />
                        ))}
                    </RadioGroup>
                </FormControl>
            </DrawerContentCard>
            ...
            <DrawerContentCard title='Price'>
                <FormControl component='fieldset'>
                    <RadioGroup aria-label='sortByPrice' defaultValue='none' name='price-name'>
                        {PRICE_SORT.map((el, idx) => (
                            <FormControlLabel key={el.val + idx} value={el.val} control={<Radio />} label={el.title} />
                        ))}
                    </RadioGroup>
                </FormControl>
            </DrawerContentCard>
